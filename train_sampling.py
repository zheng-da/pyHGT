...
def get_oag_cs():
    g = pickle.load(open('OAG_CS/graph.pkl', 'rb'))
    feat = th.tensor(np.load('OAG_CS/paper_emb.npy'))
...
if __name__ == '__main__':
    ...
    train_nids = rand_perm[:int(g.number_of_nodes() * 0.8)]
    val_nids = rand_perm[len(train_nids):int(g.number_of_nodes() * 0.9)]
    test_nids = rand_perm[int(g.number_of_nodes() * 0.9):]

    train_mask = np.zeros((g.number_of_nodes(),), dtype=bool)
    train_mask[train_nids] = True
    val_mask = np.zeros((g.number_of_nodes(),), dtype=bool)
